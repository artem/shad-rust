...
fn test_random_insertions_small() {
    ...
    for _ in 0..1000 {
        ...
        for _ in 0..10 {
            assert_eq!(cbst_set.len(), hash_set.len());
            for x in -10..10 {
                assert_eq!(cbst_set.contains(x), hash_set.contains(&x));
            }

            let y = rng.gen_range(-10..10);
            assert_eq!(cbst_set.insert(y), hash_set.insert(y));
        }
    }
}
...
fn test_random_insertions_big() {
    ...
    for _ in 0..100 {
        ...
        for _ in 0..100 {
            ...
            let x = rng.gen::<i64>();
            assert_eq!(cbst_set.insert(x), hash_set.insert(x));
        }
    }
}
...
fn test_random_removals_small() {
    ...
    for _ in 0..1000 {
        ...
        for _ in 0..20 {
            let x = rng.gen_range(-10..10);
            assert_eq!(cbst_set.remove(x), hash_set.remove(&x));
            assert_eq!(cbst_set.len(), hash_set.len());

            for y in -10..10 {
                assert_eq!(cbst_set.contains(y), hash_set.contains(&y));
            }
        }
    }
}
